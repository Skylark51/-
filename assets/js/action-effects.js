...
export function initActionEffects(app, { vibrate = false } = {}) {
  ...
  function play(name, detail = {}) {
    ...
    const word = detail.word || (name.includes("water") ? "?γ끉釉??" : choose(HIT_WORDS)); piece("impact-word", word);
  ...
  function wrong(timeout = false) { const name = timeout ? "leak-burst" : choose(WRONG_ACTIONS); sound(timeout ? "timeout" : "wrong"); play(name, { word: timeout ? "??볦퍢 ???癒?뼄!" : name === "nope" ? "??욧탷 ?袁⑤빍筌왖!" : undefined }); }
  function rare(){if(reduced()||Math.random()>.035)return;const events=[["rare-crown","?類ｋあ?곗눖???源놁삢!"],["rare-fish","?얠눊?ф묾怨뺣뼄!"],["rare-ufo","??볧뜗 UFO!"]],event=choose(events);app.classList.add(event[0]);piece("rare-prop",event[1]);setTimeout(()=>app.classList.remove(event[0]),1000)}
  ...
  addEventListener("fever:start", event => { sound("fever-start"); const tier=event.detail?.tier||event.detail?.level||1; app.dataset.feverTier=String(tier); banner.textContent=`??룻닊 ?癒?낵??FEVER ${tier}`; banner.classList.add("is-visible"); });
