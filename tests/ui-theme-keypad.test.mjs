...
test("모바일 정수 키패드는 4줄 3열 계약을 지킨다",async()=>{const code=await readFile(new URL("../assets/js/mobile-keypad.js",import.meta.url),"utf8");assert.match(code,/DIGITS=\["1","2","3","4","5","6","7","8","9"\]/);assert.match(code,/make\("C"/);assert.match(code,/make\("0"/);assert.match(code,/make\("제출"/);assert.doesNotMatch(code,/make\("⌫"|make\("전체 지우기"/)});
