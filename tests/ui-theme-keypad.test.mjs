...
test("입력 선택지는 문항이 바뀔 때만 다시 만든다",async()=>{const code=await read("../assets/js/ui-adapter.js");assert.match(code,/renderedInputKey/);assert.match(code,/if\(key===this\.renderedInputKey\)return/);assert.match(code,/descriptor\.choices/)});
test("퀴즈 반응형 CSS는 모바일 툴바와 safe area를 함께 관리한다",async()=>{const css=await read("../assets/css/quiz-screen.css");assert.match(css,/--quiz-toolbar-height/);assert.match(css,/env\(safe-area-inset-bottom\)/);assert.match(css,/\.game-bottom-toolbar/);assert.match(css,/\.question-card h2\{[^}]*overflow:auto/);assert.doesNotMatch(css,/\.game-shell\{[^}]*transform:\s*scale/)});
test("직접 게임 진입과 결과 화면에 로비 경로가 있다",async()=>{const code=await read("../assets/js/ui-effects.js");assert.match(code,/if\(!query&&!selection\?\.trainingId\)/);assert.match(code,/location\.replace\(\"index\.html\"\)/);assert.match(code,/장독대 고르기로/)});
