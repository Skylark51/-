...
test("원자량은 1~20 전체와 0.1 오차를 지원한다",()=>{assert.equal(atomicMassQuestions.length,20);const chlorine=atomicMassQuestions.find(q=>q.prompt.includes("Cl"));assert.equal(evaluateAnswer(chlorine,"35.45").correct,true);assert.equal(evaluateAnswer(chlorine,"35.6").correct,true)});
test("산화환원은 1·2 선택과 자동 제출 메타데이터를 제공한다",()=>{assert.ok(redoxQuestions.length>=12);for(const q of redoxQuestions){const input=getInputDescriptor(q);assert.equal(q.type,"binary_choice");assert.deepEqual(input.keyboardShortcuts,["1","2"]);assert.equal(input.autoSubmit,true);assert.equal(evaluateAnswer(q,q.correctChoice).correct,true)}});
